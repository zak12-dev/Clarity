        ...
        <div class="form-wrap">
          <UCard class="form-card">
            <div class="form-grid">
              ...
              <div class="field">
                <label class="field-label"
                  >Statut <span class="required">*</span></label
                >
                <USelectMenu
                  v-model="form.statusId"
                  :items="statusOptions"
                  placeholder="Choisir un statut"
                  :color="errors.statusId ? 'error' : undefined"
                  option-attribute="label"
                  value-attribute="value"
                   by-value
                />
                <p v-if="errors.statusId" class="field-error">
                  {{ errors.statusId }}
                </p>
              </div>
              ...
              <div class="field">
                <label class="field-label"
                  >Priorité <span class="required">*</span></label
                >
                <USelectMenu
                  v-model="form.privilegeId"
                  :items="privilegeOptions"
                  placeholder="Choisir une priorité"
                  :color="errors.privilegeId ? 'error' : undefined"
                  option-attribute="label"
                  value-attribute="value"
                   by-value
                />
                <p v-if="errors.privilegeId" class="field-error">
                  {{ errors.privilegeId }}
                </p>
              </div>
...
const { data: privileges } = await useFetch<Privilege[]>("/api/privileges", {
  default: () => [],
});
...
if (isAdmin.value) {
  const { data } = await useFetch<{ id: string; name: string }[]>(
    "/api/users",
    {
      default: () => [],
    },
